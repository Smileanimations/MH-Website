    ...
    <div class="p-3">
        <div class="contianer">
            <div class="">
                ...
                <div class="mt-5">
                    <img src="Images/locales/<?= str_replace(" ", "_", $locale['name'])?>.webp" alt="" style="width: 600px;">
                    <div class="float-end float-top text-center mb-5">
                        <h2>Map</h2>
                        <img src="Images/locale_icons/<?= str_replace(" ", "_", $locale['name'])?>_Map.webp" alt="No Map Available" style="width: 300px;">
                    </div>
                </div>
            </div>
        </div>
...
<?php 
$sqlmonsters = "SELECT * FROM monsters";
$stmt = $connection->query($sqlmonsters);
        ...
        <?php $localemonsters = explode(", ", $locale['monsters']);
            foreach ($localemonsters as $localemonster) { 
                foreach ($monsters as $index => $monster) {
                    if ($monster['name'] == $localemonster) {?>
                <div class="col-sm-3">
                    <div class="p-3">
                        <a href="details.php?id=<?= $monster['id']?>"><img src="Images/icons/<?= str_replace(" ", "_", $localemonster)?>.webp" alt="" class="icons"></a>
                        <p><?= $localemonster?></p>
                    </div>
                </div>
            <?php }}}?>
